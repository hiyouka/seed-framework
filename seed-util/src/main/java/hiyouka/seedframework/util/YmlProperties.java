...
public class YmlProperties extends Hashtable<String,String> {
            ...
            while((line = lineReader.readLine()) != null){
                ...
                if(!needSkip(line)){
                    ...
                    if(lineWrapper.hasValue()){
                        this.setProperty(lineWrapper.getPropertiesKey().toString(),lineWrapper.getValue());
                    }
    ...
    static class LineWrapper{
        ...
        public boolean higher(LineWrapper lineWrapper){
            return this.rank < lineWrapper.getRank();
        }

        public boolean lower(LineWrapper lineWrapper){
            return this.rank > lineWrapper.getRank();
        }

        public boolean isSameRank(LineWrapper lineWrapper){
            return this.rank == lineWrapper.getRank();
        }
        ...
        public void setSuperiorLine(LineWrapper lineWrapper){
            if(isTop()){
                return;
            }
        ...
        public LineWrapper getSuperiorLine(){
            return this.superiorLine;
        }

        public boolean isSuperLine(LineWrapper lineWrapper){
            if(isTop()){
                return false;
            }
            if(this.superiorLine != null){
                String propertiesKey = this.superiorLine.getPropertiesKey().toString();
                if(StringUtils.hasText(propertiesKey)){
                    return propertiesKey.equals(lineWrapper.getPropertiesKey().toString());
                }
            }
            return false;
        }
